...
/* Build init or upgrade dbschema from directory data
   errExit argument means return error when sql format found
*/
func Build(db *sql.DB, dir string, errExit bool) error {
	defer logging.Printf("[INFO] build schema done")
	err := prepare(db)
	if err != nil {
		return err
	}
	...
	for _, table := range tables {
		...
		if err != nil {
			logging.Printf("[WARN] stat for %s directory failed, err=%v", table, err)
			continue
		}
		if !fi.IsDir() {
			logging.Printf("[WARN] directory %s is not directory, skiped", table)
			continue
		}
		err = buildDir(db, table, info[path.Base(table)])
		if err != nil {
			logging.Printf("[ERROR] build schema for %s failed, err=%v", path.Base(table), err)
			if errExit {
				return err
			}
			continue
...
func buildDir(db *sql.DB, dir string, tb table) error {
	logging.Printf("[INFO] upgrading table %s...", path.Base(dir))

	buildDir := path.Join(dir, "schema")
	fi, err := os.Stat(buildDir)
	if os.IsNotExist(err) || !fi.IsDir() {
		logging.Println("  * no schema found, skiped")
	} else {
	...
	if os.IsNotExist(err) || !fi.IsDir() {
		logging.Println("  * no data found, skiped")
	} else {
